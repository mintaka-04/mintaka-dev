...
### 엔드포인트
기록 생성(`memories POST`) → 상세 조회(`memory/[id] GET`) 세트를 VU당 반복한다.

생성 후 앱이 바로 상세 페이지로 이동하는 실제 흐름(`router.replace('/memory/${newId}')`)을 그대로 재현한 것이다. 삭제/수정은 사용 빈도가 낮고 부하 특성이 유사해 제외했다.
...
부하 전 VU 1~2로 정상 응답시간을 먼저 측정하고 부하 시와 비교한다.

### WebSocket 연결 상태 관찰

k6로 직접 모니터링하지 않고, CloudWatch 로그에서 watchdog 재연결 로그(`⚠️ Realtime 연결 끊김 감지`) 발생 여부로 관찰한다.
